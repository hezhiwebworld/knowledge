...
function generator(pwd, source_file_name, dest_file_path, is_open, options) {
  ...
  function copy_images_folder(folder){
    if (fs.existsSync(pwd + '/' + folder)){
      console.log('exist images folder = ' + folder)
      cp('-R', pwd +'/' + folder, preview_path + '/' );
    }
  }
  
  function copy_images_folders(folders){
    for (var i = 0; i < folders.length; i++) {
      var folder = folders[i]
      copy_images_folder(folder)
    } 
  }
    ...
    var rs = fs.createReadStream(template_path, {bufferSize: 11}); 
        ...
        copy_images_folders(['images', 'image', 'img', 'asset', 'assets'])
